...
const ItemsPage = () => {
	...
	const getItemCategories = async () => {
		const response = await axios({
			method: "get",
			url: "/ItemCategories/getItemCategories/" + localStorage.getItem("organization_uuid"),
		...
		() =>
			setFilterItemsData(
				itemsData
					...
					.filter(
						a => !filterCategory || a.category_title.toLocaleLowerCase().includes(filterCategory.toLocaleLowerCase())
					)
			),
	...
	return (
		<>
			...
			{deletePopup ? (
				<DeleteItemPopup onSave={() => setDeletePopup(false)} setItemsData={setItemsData} popupInfo={deletePopup} />
			) : (
				""
			)}
			{pictureUploadPopup ? (
...
function Table({ itemsDetails, setPopupForm, setDeletePopup, setPictureUploadPopup }) {
		...
		<table className="user-table" style={{ maxWidth: "100vw", height: "fit-content", overflowX: "scroll" }}>
			...
			<tbody className="tbody">
				{itemsDetails
					...
					?.map((item, i) => (
						<tr key={Math.random()} style={{ height: "30px" }} onClick={() => {}}>
							<td>{i + 1}</td>
										...
										<div className="flex" style={{ justifyContent: "space-between" }}>
											<div className="flex">
												<input
													...
													checked={+data?.status}
													style={{ height: "25px" }}
													onClick={() => setdata(prev => ({ ...prev, status: 1 }))}
												/>
												On
											</div>
											<div className="flex">
												<input
													...
													checked={!+data.status}
													style={{ height: "25px" }}
													onClick={() => setdata(prev => ({ ...prev, status: 0 }))}
												/>
												Off
											</div>
										</div>
				...
				>
					<div style={{ overflowY: "scroll" }}>
						<form className="form" onSubmit={submitHandler}>
							...
							<div className="formGroup">
								<div className="row">
									<label htmlFor="upload_image" className="selectLabel file_upload">
										<h2>
											<Image />
											Upload Image
										</h2>
									...
									{images ? (
										<div className="imageContainer">
											<img src={URL.createObjectURL(images.file)} className="previwImages" alt="yourimage" />
											<button
							...
							{deleteImages.length ? (
								<button type="button" className="submit" onClick={deleteSubmitHandler}>
									Save
								</button>
							) : (
								""
							...
							{editImages ? (
								<button type="button" className="submit" onClick={editSubmitHandler}>
									Update
								</button>
							) : (
								""
...
function DeleteItemPopup({ onSave, popupInfo, setItemsData }) {
	...
	const submitHandler = async e => {
		...
		try {
			...
			if (response.data.success) {
				setItemsData(prev => prev.filter(i => i.item_uuid !== popupInfo.item_uuid))
				onSave()
			}
		} catch (err) {
			console.log(err)
